 ...
 * build date: 2023-07-13T08:08:34.757Z 
 */
...
(function(module, exports) {
  !function(e2, o2) {
    ...
    function e2(e3, o3) {
      ...
      return n3.addListener(i2), function() {
        return n3.removeListener(i2);
      };
    }
    function o2(e3, o3) {
    ...
    return t2.csp = t2.uwp || false, t2.nwjs ? t2.dev = "sdk" === process.versions["nw-flavor"] : t2.electron ? t2.dev = process.execPath.replace(/\\/g, "/").includes("node_modules/electron/") : t2.uwp ? t2.dev = Windows.ApplicationModel.Package.current.isDevelopmentMode : t2.node ? t2.dev = "production" !== process.env.NODE_ENV : t2.dev = void 0, t2;
  });
})(platformDetect);
...
function IdentifyContent(checkedElement) {
...
function PatchContentSize(checkedElement, width, height) {
...
var OidcClient = class {
  ...
  async processSigninResponse(url2) {
    ...
    const { state, response } = await this.readSigninResponseState(url2, true);
  ...
  async processSignoutResponse(url2) {
    ...
    const { state, response } = await this.readSignoutResponseState(url2, true);
    if (state) {
      logger2.debug("Received state from storage; validating response");
...
var UserManager = class {
  ...
  async signinCallback(url2 = window.location.href) {
    const { state } = await this._client.readSigninResponseState(url2);
  ...
  async signoutCallback(url2 = window.location.href, keepOpen = false) {
    const { state } = await this._client.readSignoutResponseState(url2);
    if (!state) {
      return;
    }
    switch (state.request_type) {
...
class SecureCitizenBootstrapper {
   ...
   */
  constructor(sourceDiv, clientId, mask) {
    ...
    const videoElement = document.getElementById("cameraVideo" + this.random_id_suffix);
    const canvasElement = document.getElementById("cameraVideo" + this.random_id_suffix);
